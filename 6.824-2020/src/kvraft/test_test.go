...
import "6.824/labs/porcupine"
import "6.824/labs/models"
import "testing"
import "strconv"
import "time"
import "math/rand"
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			for atomic.LoadInt32(&done_clients) == 0 {
				if (rand.Int() % 1000) < 500 {
					...
					log.Printf("%d: client end append %v\n", cli, nv)
					last = NextValue(last, nv)
					j++
				} else {
					log.Printf("%d: client new get %v\n", cli, key)
					v := Get(cfg, myck, key)
					log.Printf("%d: client end get %v\n", cli, key)
					if v != last {
...
func TestSnapshotRPC3B(t *testing.T) {
	...
	fmt.Printf("majority partition begin")
	...
	fmt.Printf("new partition begin")
