...
func TestRejoin2B(t *testing.T) {
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf("leader2 disconnect")
	// old leader connected again
	cfg.connect(leader1)
	DPrintf("leader1 connect")
	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	DPrintf("leader connect")
...
func TestFigure82C(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		if nup < 3 {
			...
			DPrintf("all crash server reboot: ")
		}
	}
...
func snapcommon(t *testing.T, name string, disconnect bool, reliable bool, crash bool) {
	...
	for i := 0; i < iters; i++ {
		...
		if disconnect {
			...
			DPrintf("rf[%v] disconnect",victim)
			cfg.one(rand.Int(), servers-1, true)
		}
		if crash {
			cfg.crash1(victim)
			DPrintf("rf[%v] crash",victim)
			cfg.one(rand.Int(), servers-1, true)
		}
